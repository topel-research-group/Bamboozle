...
barcode = subparsers.add_parser("barcode", parents=[input_commands, other_commands, threshold_command], \
				usage="bamboozle.py barcode -f <ref> -b <bam1> <bam2> ... <bamN> -o <prefix> {--haploid | --diploid} [options]", \
...
barcode.add_argument("--haploid", action="store_true", \
			help="Find barcodes in haploid samples. Mutually exclusive with --diploid")
barcode.add_argument("--diploid", action="store_true", \
			help="Find barcodes in diploid samples. Mutually exclusive with --haploid")
...
#				cProfile.runctx('cl.main(args)', globals(), locals(), 'long_coverage.prof')
		...
		if args.haploid and args.diploid:
			sys.exit("[Error] Please use either --haploid or --diploid, not both.")
		if args.haploid == False and args.diploid == False:
			sys.exit("[Error] Please specify whether your samples are --haploid or --diploid.")
